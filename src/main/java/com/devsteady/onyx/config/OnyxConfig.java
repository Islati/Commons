...
@SerializeOptions(
		configHeader = {
				...
				"are described under the Wiki on Onyx GitHub page."
		},
		configMode = ConfigMode.DEFAULT
)

public class OnyxConfig extends YamlConfig implements Configuration {
	@Path("Commands.register-commands")
	@Comments({
			"By default Onyx includes a plethora of commands",
			"Designed to aid you in your server ventures!",
			"Though if you're not requiring use of these commands, and",
			"Wish to use Onyx for only its API Features, then change this value to",
			"False"
	})
	...
	@Comments({
			"Determines whether or not Onyx should",
