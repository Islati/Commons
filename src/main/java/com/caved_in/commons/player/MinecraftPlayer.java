	...
	public static class TeleportRequest {
		...
		public void accept(Player accepting) {
			switch (type) {
				 ...
				 */
				case TELEPORT_TO:
					Player toRequested = Players.getPlayer(receiver);
					commons.debug("<TP> Player " + accepting.getName() + " teleported to " + toRequested.getName());
					Players.teleport(accepting, toRequested);
					Chat.message(accepting, Messages.playerTeleportedToPlayer(toRequested.getName()));
					Chat.message(toRequested, Messages.playerTeleportedToYou(accepting.getName()));
					filled = true;
					break;
				case TELEPORT_HERE:
					Player hereRequested = Players.getPlayer(receiver);
					commons.debug("<TP-HERE> Player " + hereRequested.getName() + " teleported to " + accepting.getName());
					Players.teleport(hereRequested, accepting);
