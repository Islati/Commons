	...
	public void onTeleportPositionCommand(Player player, String[] args) {
		if (args.length >= 3) {
			...
			if (StringUtils.isNumeric(xArg) && StringUtils.isNumeric(yArg) && StringUtils.isNumeric(zArg)) {
				PlayerHandler.teleport(player, new double[]{Double.parseDouble(xArg), Double.parseDouble(yArg), Double.parseDouble(zArg)});
			} else {
				PlayerHandler.sendMessage(player, Messages.INVALID_COMMAND_USAGE("x", "y", "z"));
			}
		} else {
			PlayerHandler.sendMessage(player, Messages.INVALID_COMMAND_USAGE("x", "y", "z"));
		}
