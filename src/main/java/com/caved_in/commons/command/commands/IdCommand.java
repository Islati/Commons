...
public class IdCommand {
    ...
    @Flags(identifier = "o")
    public void onIdCommand(Player player, @Arg(name = "firstPageEnabled", def = "0") String item, @FlagArg("o")final boolean offHand) {
        if (item == null || "0".equals(item)) {
            if (!Players.hasItemInHand(player)) {
                Chat.message(player, "&eEither &ogive an firstPageEnabled name as an argument, &e&lor&r&e &ohold an firstPageEnabled in either of your hands&r&e.");
                return;
            }
